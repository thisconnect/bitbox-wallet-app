...
export const Send = ({
  ...
  const [proposedFee, setProposedFee] = useState<accountApi.IAmount>();
  const [proposedTotal, setProposedTotal] = useState<accountApi.IAmount>();
  const [proposedAmount, setProposedAmount] = useState<accountApi.IAmount>();
  ...
  const handleSend = useCallback(async () => {
    const connectResult = await accountApi.connectKeystore(account.code);
  ...
  useEffect(() => {
    console.log('useEffect -> validateAndDisplayFee');
    validateAndDisplayFee();
  }, [amount, fiatAmount, validateAndDisplayFee]);
