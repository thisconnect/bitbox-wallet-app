	...
	for _, account := range accounts {
		...
		for txID, note := range notesData.TransactionNotes {
			entry := bip329Entry{
				Type:  bip329TypeTx,
				Ref:   txID,
				Label: util.TruncateString(note, notes.MaxNoteLen),
...
// ExportNotes exports the transaction and account labels of all accounts of all
...
func (backend *Backend) ImportNotes(jsonLines []byte) (*ImportNotesResult, error) {
	...
	for scanner.Scan() {
		...
		case bip329TypeTx:
			...
			// It is inefficient to store dump all notes to disk every for every imported note,
